...
(deftest validate-data
  ...
  (let [sc  {:company {:db/valueType :db.type/string}
    ...
    (is (thrown-with-msg?
          Exception #"Invalid data, expecting"
          (d/db-with db [{:db/id -2 :company 1 :id (random-uuid) :code 1}])))
    (is (thrown-with-msg?
          Exception #"Invalid data, expecting"
          (d/db-with db [{:db/id -3 :company :abc :id (random-uuid) :code 1}])))
    (is (thrown-with-msg?
          Exception #"Invalid data, expecting"
          (d/db-with db [{:db/id -4 :company 1.0 :id (random-uuid) :code 1}])))
    (is (thrown-with-msg?
          Exception #"Invalid data, expecting"
          (d/db-with db [{:db/id -5 :company "XYZ" :id (random-uuid) :code "1"}])))
    (d/close-db db)
    (u/delete-files dir)))
