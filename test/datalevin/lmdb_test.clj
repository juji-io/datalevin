  ...
  (let [dir  (u/tmp-dir (str "kv-wal-default-" (UUID/randomUUID)))
    ...
    (try
      ...
      (is (nil? (if/get-value lmdb c/kv-info
                              c/last-committed-wal-tx-id :attr :long)))
      (is (nil? (if/get-value lmdb c/kv-info
                              c/wal-next-tx-id :attr :long)))
      (is (not (contains? (set (if/list-dbis lmdb)) c/kv-wal)))
      (finally
        (if/close-kv lmdb)
        (u/delete-files dir)))))
...
(deftest kv-wal-append-test
  (let [dir  (u/tmp-dir (str "kv-wal-append-" (UUID/randomUUID)))
    ...
    (try
      ...
      (if/transact-kv lmdb [[:put "a" 1 "x"]
                            [:put "a" 2 "y"]])
      (let [wal-id   (if/get-value lmdb c/kv-info
                                   c/last-committed-wal-tx-id :attr :long)
            next-id  (if/get-value lmdb c/kv-info c/wal-next-tx-id :attr :long)
            idx-id   (if/get-value lmdb c/kv-info
                                   c/last-indexed-wal-tx-id :attr :long)
            user-id  (if/get-value lmdb c/kv-info
                                   c/last-committed-user-tx-id :attr :long)
            payload  (if/get-value lmdb c/kv-wal wal-id :id :raw)
            record   (nippy/thaw payload)]
        (is (= 1 wal-id))
        (is (= wal-id next-id))
        (is (= wal-id idx-id))
        (is (= wal-id user-id))
        (is (= :kv (:wal/record-type record)))
      ...
      (is (= 2 (if/get-value lmdb c/kv-info
                             c/last-committed-wal-tx-id :attr :long)))
      (is (contains? (set (if/list-dbis lmdb)) c/kv-wal))
      (finally
        (if/close-kv lmdb)
        (u/delete-files dir)))))
...
(deftest kv-wal-replay-test
  (let [dir  (u/tmp-dir (str "kv-wal-replay-" (UUID/randomUUID)))
    ...
    (try
      ...
      (is (= 2 (if/get-value lmdb c/kv-info c/wal-next-tx-id :attr :long)))
  ...
  (let [dir  (u/tmp-dir (str "kv-wal-guard-" (UUID/randomUUID)))
    ...
    (try
      (let [ex (try
        ...
        (is (= :access/unauthorized (:error (ex-data ex)))))
      (finally
        (if/close-kv lmdb)
        (u/delete-files dir)))))
...
(deftest kv-wal-replay-bounded-and-noop-test
  (let [dir   (u/tmp-dir (str "kv-wal-replay-bounded-" (UUID/randomUUID)))
    ...
    (try
      ...
      (is (= {:from 0 :to 1 :applied 1}
             (l/replay-kv-wal! lmdb 1)))
      (is (= 1 (if/get-value lmdb c/kv-info
                             c/last-indexed-wal-tx-id :attr :long)))

      (is (= {:from 1 :to 1 :applied 0}
             (l/replay-kv-wal! lmdb 1)))
