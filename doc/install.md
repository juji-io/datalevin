...
```Clojure
{:deps {datalevin/datalevin {:mvn/version "0.7.12"}
...
A JVM
[uberjar](https://github.com/juji-io/datalevin/releases/download/0.7.12/datalevin-0.7.12-standalone.jar)
...
```console
java --add-opens=java.base/java.nio=ALL-UNNAMED --add-opens=java.base/sun.nio.ch=ALL-UNNAMED -jar datalevin-0.7.12-standalone.jar
```
This will start the Datalevin REPL.

```console
java --add-opens=java.base/java.nio=ALL-UNNAMED --add-opens=java.base/sun.nio.ch=ALL-UNNAMED -jar datalevin-0.7.12-standalone.jar serv -r /tmp/test-server
```
